...
def parse_arguments():
    ...
    parser.add_argument('--check_emission', action='store_true', help='If set, check for emission textures')
...
def bake_texture(high_obj, low_obj, bake_type, cage_dist):
    ...
    bpy.context.scene.cycles.adaptive_threshold = 0.01  # Lower = more samples where needed

    # Denoising OFF (can blur details)
    bpy.context.scene.cycles.use_denoising = False
    
    # Anti-aliasing settings
    bpy.context.scene.cycles.pixel_filter_type = 'BLACKMAN_HARRIS'  # Best quality filter
    bpy.context.scene.cycles.filter_width = 1.5  # Smooth edges
...
def apply_emission_texture(model_obj, image_path):
    """
    Load an image from disk and connect it to the Diffuse input
    of the Principled BSDF shader for the model's material.
    ...
    tex_node = nodes.new("ShaderNodeTexImage")
    tex_node.image = bpy.data.images.load(image_path)
    tex_node.label = "Emission Texture"
    tex_node.name = "Emission_Texture"
...
def export_emission_texture(obj, output_dir, filename="emission.png", image_size=512):
    """
    Check all materials of the object for an image texture connected to Emission,
    and save it to output_dir.

    Args:
        obj (bpy.types.Object): The Blender object to check.
        output_dir (str): Folder where to save the texture.
        filename (str): Name of the saved image file.

    Returns:
        bool: True if an emission texture was found and saved, False otherwise.
    """
    if obj.type != 'MESH':
        return False
            ...
            if node.type == 'EMISSION':
                ...
                if node.inputs['Color'].is_linked:
                    ...
                    if from_node.type == 'TEX_IMAGE' and from_node.image:
                        ...
                        image.scale(image_size, image_size)
                        image.save()
                        found = True
                        return True  # Exit after first found
...
if __name__ == "__main__":
    ...
    print (f"---> cage_distance = {cage_distance}")
    ...
    if (args.check_emission):
        if (export_emission_texture(high_model, out_dir, image_size=args.image_size)):
            apply_emission_texture(low_model, os.path.join(out_dir,  "emission.png"))
